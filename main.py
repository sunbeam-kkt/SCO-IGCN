...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward
            logit_ABS, logit_MoT, concat_ABS_MoT_logit = self.model(data)  # 0.5, 0.5, 0.3, 1
            # print("logit shape:", logit_ABS.shape)
            predict_logit = 0.5 * logit_ABS + 0.5 * logit_MoT + concat_ABS_MoT_logit

            l1 = 0 
            loss = self.loss(predict_logit, label) + l1
            ...
            value, predict_label = torch.max(predict_logit.data, 1)
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    ...
                    label = Variable(
                        ...
                        requires_grad=False,
                        volatile=True)
                    logit_ABS, logit_MoT, concat_ABS_MoT_logit = self.model(data)
                    predict_logit = 0.5 * logit_ABS + 0.5 * logit_MoT + concat_ABS_MoT_logit
                    if isinstance(predict_logit, tuple):
                        output, l1 = output
                        l1 = l1.mean()
                    else:
                        l1 = 0
                    loss = self.loss(predict_logit, label)
                    score_frag.append(predict_logit.data.cpu().numpy())
                    loss_value.append(loss.data.item())

                    _, predict_label = torch.max(predict_logit.data, 1)
                    step += 1
