...
def pre_normalization(data, zaxis=[0, 1], xaxis=[8, 4]):
    ...
    for i_s, skeleton in enumerate(tqdm(s)):  # 遍历每个batch，skeleton：[M, C, T, V], i_s为每个样本ID
        ...
        def insert_frames(frames, target_frames=150):
            fo = len(frames)
            fo1 = fo - 1
            if fo >= target_frames:  # 如果fo大于等于150
                # drop_frame = fo - target_frames  # 应该丢弃的帧数
                frame = frames[:target_frames, :, :]
                return frame  # 如果原始帧数已经大于等于目标帧数，直接返回原序列的后150帧
            else:
                DM = np.zeros((fo1, V, C))  # 帧间位移矩阵
                FF = np.zeros((300, V, C))  # 填充帧矩阵
                ff = (target_frames - 1) // fo  # 计算每两帧之间需要插入的帧数
                fd = (target_frames - 1) % fo  # 计算需要丢弃的帧数
                    ...
                    for v in range(V):
                        for c in range(C):
                            ...
                            for f in range(ff):
                                ...
                                FF[f + t * ff, v, c] = Nc
                                if (fd == 0) or ((f + t * ff) <= (fu - 1)):
                                    FF[f + t * (ff + 1), v, c] = Nc
                                if (fd != 0) and ((f + t * ff) >= (fu - 1)):
                                    if f < (ff - fd):
                                        FF[f + t * ff, v, c] = Nc
        ...
        for i_p, person in enumerate(skeleton):  # 遍历每个人，person：[T, V, C], i_p为每个人ID
            ...
            L = t
            front = math.ceil(L * 0.2)
            rear = math.ceil(L * 0.8)
            interpolation_range = [front, rear]

            # 1. 计算所有帧的平均值
            mean_frame = person.mean(axis=0)  # Shape: [V, C]，所有输入帧的平均值
            
            # 2. 计算每一帧与平均帧的欧氏距离
            min_dist = np.linalg.norm(person[0] - mean_frame)  # 预定义
            key_frame_index = t // 2  # same as above
            ...
            if key_frame_index not in interpolation_range or L <= 10:
                if person[0].sum() == 0:
                    index = (person.sum(-1).sum(-1) != 0)
                    tmp = person[index].copy()
                    person *= 0
            ...
            #     before_key_frame = person
            #     after_key_frame = person
            # else:
                before_key_frame = person[:key_frame_index]
                after_key_frame = person[key_frame_index:]
